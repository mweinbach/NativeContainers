...
| Docker Compose | Private official client + optional Docker CLI bridge + conformance/topology/live-fixture services | M2 | Docker Compose 5.1.4 is version/digest/provenance pinned and installed only in private app support; normal teardown and Apple-native force cleanup are live-proven through that exact client. Aliases remain partial, health/restart/config/secret parity is unsupported, and user-project lifecycle stays blocked pending a reviewed desired-state parser |
