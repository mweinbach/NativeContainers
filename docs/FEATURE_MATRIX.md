...
| Docker Compose | Review/planning/revalidation/journal/mutation services + private official client + optional Docker CLI bridge | M2 | Docker Compose 5.1.4 is version/digest/provenance pinned. Fresh Up and exact-ID Start, Stop, and declared-service Down are enabled only after stable redacted review and commit-time source/binary/environment/inventory revalidation; cancellation leaves a manual-only durable recovery record. Existing-project convergence, aliases, health/restart/config/secret parity, orphan deletion, and named-volume deletion remain blocked. Apple 1.0 mutation APIs have no conditional identity token, so an external client can still race the final revalidation-to-mutation gap. |
