...
| Docker Compose | Optional Docker CLI bridge + pure Apple-inventory topology service | M2 | Read-only canonical project/service/volume/network observability, Quick Open, status, and exact-resource cross-links are live; execution remains experimental and project lifecycle awaits explicit conformance fixtures |
