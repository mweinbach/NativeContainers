...
| VM storage reclamation | Saved-state store + descriptor-safe residue inspector + category services + app-scoped review model | M4 | Reviewed committed saved states and exact app-owned interrupted-operation residue are live with accounting/library provenance, operation/runtime locks, device/inode/tree seals, hard-link/symlink/mount-crossing rejection, atomic retirement, and partial results. Active or changed items are preserved; the path never starts, stops, force-stops, or kills a VM and never touches VM disks or restore images. |
...
| Docker Compose | Review/planning/revalidation/journal/mutation services + private official client + optional Docker CLI bridge | M2 | Docker Compose 5.1.4 is version/digest/provenance pinned. Exact-count existing-project Up, exact-ID Start/Stop, and reviewed declared-container/orphan/network/named-volume Down are enabled only after stable redacted review and commit-time source/binary/environment/inventory revalidation; cancellation leaves a manual-only durable recovery record. Create-missing and recreation remain blocked pending a deterministic external-resource overlay, stable Compose metadata paths, and an explicit supported-key allowlist; alias, health, restart, config, and secret parity remain blocked. Apple 1.0 mutation APIs have no conditional identity token, so an external client can still race the final revalidation-to-mutation gap. |
