...
| Docker Compose | Optional Docker CLI bridge + pure conformance/topology services | M2 | Read-only canonical project/service/volume/network observability and a source-pinned 1.0.0 capability report are live; aliases are partial, health/restart/config/secret parity is unsupported, and project lifecycle remains policy-blocked pending reviewed-model and isolated live-wire fixtures |
